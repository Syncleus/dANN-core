...
public abstract class AbstractPath<N, E extends TraversableEdge<N>> extends AbstractWalk<N, E> implements Path<N, E>
{
	@Override
	...
	static <N, E extends TraversableEdge<N>> boolean verifyUtility(final List<N> nodeSteps, final List<E> edgeSteps)
	{
		if( nodeSteps.size() < 2 )
	...
	protected static <N, E extends TraversableEdge<N>> boolean isChain(final Path<N, E> path)
	{
	...
	static <N, E extends TraversableEdge<N>> boolean isIndependentUtility(final Path<N, E> firstPath, final Path<N, E> secondPath)
	{
