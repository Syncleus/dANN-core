 ...
 **/
public class EigenvalueDecomposition implements java.io.Serializable, Decomposition<RealMatrix>
{
	/** Symmetry flag.
	 ...
	 */
	private List<RealNumber> nonsymmetricOrt;
	 ...
	 */
	public EigenvalueDecomposition(RealMatrix matrixToDecompose)
	{
		double[][] A = matrixToDecompose.toDoubleArray();
		...
		this.nonsymmetricOrt = new ArrayList<RealNumber>(ort.length);
		for(double ortValue : ort)
			this.nonsymmetricOrt.add(new RealNumber(ortValue));
	...
	private void hessenbergReduction()
	{
		int n = this.getDimensionSize();
		double[][] V = this.matrix.toDoubleArray();
		double[][] H = this.hessenbergMatrix.toDoubleArray();
		double[] ort = new double[this.nonsymmetricOrt.size()];
		for(int valueIndex = 0; valueIndex < ort.length; valueIndex++)
			ort[valueIndex] = this.nonsymmetricOrt.get(valueIndex).getValue();
